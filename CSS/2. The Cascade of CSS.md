...
2. You can chain the `.child` with a selector! like `p.child` [[1. Introduction to CSS#Type Selectors (div, p, etc.)|(Type Selector)]] or `.para.child` [[1. Introduction to CSS#Class Selectors|(Class Selectors)]] **But here** it is better to chain it with `p.child` because it makes the most sense, as a .child can be any child, but it will always be a paragraph `p`.
...
## Another mistake
 [Between a rule that uses one class selector and a rule that uses three type selectors, which rule has the higher specificity?](https://www.theodinproject.com/lessons/foundations-the-cascade#high-specificity-class-type)
 
- Class selector (`.class`) = **0,0,1,0**
- Type selector (`div`, `p`, etc.) = **0,0,0,1**

So:

- One class selector → **0,0,1,0**
- Three type selectors → **0,0,0,3**